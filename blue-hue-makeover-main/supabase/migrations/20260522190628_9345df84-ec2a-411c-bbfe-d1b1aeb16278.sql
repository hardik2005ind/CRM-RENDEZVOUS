...
DROP TABLE IF EXISTS public.lead_updates CASCADE;
...
DROP TYPE IF EXISTS public.deal_size CASCADE;
...
-- Auto-create profile + role on signup. admin@company.com -> admin, else activity_head.
...
CREATE POLICY "leads insert own" ON public.leads FOR INSERT TO authenticated
  WITH CHECK (owner_id = auth.uid());
CREATE POLICY "leads update own" ON public.leads FOR UPDATE TO authenticated
...
CREATE POLICY "lead_updates read scoped" ON public.lead_updates FOR SELECT TO authenticated
  USING (
    ...
    OR EXISTS (SELECT 1 FROM public.leads l WHERE l.id = lead_updates.lead_id AND l.owner_id = auth.uid())
  );
CREATE POLICY "lead_updates insert by owner" ON public.lead_updates FOR INSERT TO authenticated
  WITH CHECK (
    created_by = auth.uid()
    AND EXISTS (SELECT 1 FROM public.leads l WHERE l.id = lead_updates.lead_id AND l.owner_id = auth.uid())
  );
